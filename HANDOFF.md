...
├── .gitignore      # 忽略 .env
├── HANDOFF.md      # 本文件（给 AI Agent 的开发交接文档）
└── USAGE.md        # 面向最终用户的使用指南
```

**两份文档的分工**（改功能时**两份都要同步更新**）：
- `HANDOFF.md` — 给下一个 AI/开发者的技术交接：架构、代码位置、坑、待办
- `USAGE.md` — 给最终用户的使用说明：每个功能怎么用、隐私说明、FAQ
- 页面内的「使用指南」入口也指向 `USAGE.md` 的内容（见下方优化方向）
...
### 2.5 页面引导
- 右上角 `?` 或页脚可打开页面内快速使用指南，并链接到 `USAGE.md`
- 首次访问显示可关闭的新手提示，使用 `os_seen_intro` 保存在本地
- AI 图片问答面板显示提问顺序提示
...
> **重要原则**：本项目的优化有两个平行战场——**页面（index.html）** 和 **文档（HANDOFF.md / USAGE.md）**。
> 每次改动功能，都要回头检查两份文档是否过时。文档本身就是产品的一部分，不要只改代码不改文档。
...
- [ ] 结果预览：在当前页内嵌展示搜索结果，而非跳转新标签

**页面内文档入口（已完成）**
- [x] 页面内快速「使用指南」弹窗与完整 `USAGE.md` 链接
- [x] 首次访问简短提示
- [x] AI 面板加入提问顺序提示
- [x] 页脚增加使用指南与完整说明链接
...
**体验/视觉类**
- [ ] 更精致的空状态和加载动画
- [ ] 无障碍优化（键盘导航、aria 标签、对比度）
- [ ] 国际化和繁体中文适配（香港用户）

### B. 文档优化（USAGE.md / HANDOFF.md）

- [ ] **USAGE.md 配图**：为拍照识图、AI 问答、添加到主屏幕等关键步骤加截图或 GIF
- [x] USAGE.md 增加"更新日志"章节，让用户知道每次新增了什么
- [x] USAGE.md 增加"进阶技巧"章节（AI 提问模板、识图引擎组合用法）
- [ ] 把 USAGE.md 做成页面可渲染的格式，供页面内指南入口直接读取（见 A 组）
- [ ] HANDOFF.md 补充架构图（数据流：用户→识图引擎/AI→结果）
- [x] 两份文档补充"版本号"与"最后更新日期"，便于追踪
...
*文档生成时间：2026-09-19*
*本次优化：页面指南入口、首次提示、AI 使用提示与 USAGE.md 进阶技巧*
